...
function _artifact_key(planner::AbstractPlanner, mdp, context::PlanningContext)
    ...
    @match scope begin
        ...
        :initial_state => (
            :initial_state, context.hypothesis_id, hash(typeof(planner)), hash(mdp),
            isempty(context.states) ? nothing : hash(first(context.states)),
        )
        :history => (
            :history, context.hypothesis_id, context.timestep, hash(typeof(planner)),
            hash(mdp), hash(context.states), hash(context.actions),
        )
        :none => nothing
        _ => error("unknown planner cache scope: $scope")
    end
end
