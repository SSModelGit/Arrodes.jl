@gen function inference_model(
    ...
    for k in 1:config.k_components
        idx, params = @trace(
            Priors.sample_component_and_params(component_switch, component_sampler),
            k => :component
        )
    ...
    component_fns = [
        Priors.make_component(component_fields_vec[k], component_params[k]) for
        k in 1:config.k_components
    ]
...
function reconstruct_objective_from_trace(trace, config::InferenceConfig)
    ...
    component_fns = [
        Priors.make_component(component_fields[k], component_params[k]) for
        k in 1:config.k_components
    ]
