...
function plot_particle_filter_explanation(pf_state, config::InferenceConfig, 
    ...
    for (rank, particle_idx) in enumerate(top_indices)
        ...
        # Create belief updater and run simulation with learned policy
        bup = RL.beliefstate_for_pomdp(mdp)
        sim_trace = stepthrough_sim(mdp, policy, bup, n_timesteps; plot_sim_trace=false)
